...
The note, as an argument:
    ...
    $capture "the rate limiter drops the first request after an idle period"
...
Append one line to `religion/context/inbox.md` with the date and, when there is one, the file
...
- **It does not spec.** A note is not a work item. $fix turns one into a spec when you
  decide to.
...
- **It does not record code defects found by review.** Those are findings, they carry
  severities and block completion, and $audit owns them. A note is something you
...
$fix and $feature read the inbox when they start, and offer what is in it
...
- One line per note. If it needs a paragraph, it needs a spec, and that is $fix.
