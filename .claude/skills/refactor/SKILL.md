...
`religion/context/current-work.md` and stops, the same as /feature and /fix.
/implement builds it under the usual review gates.

## Input

A **target is required**, and may be any size:

    /refactor src/lib/date.ts        a single file
    /refactor src/legacy             a folder
    /refactor packages/api           one package
    /refactor .                      a monorepo root
...
A **lens** may follow, to survey only that axis: `/refactor packages/api duplication`.
...
`refactor.maxFunctionLines` in `religion/config.json`. They are defaults rather than a
...
name the target, say what the suite does cover, and point at /tests to cover the
...
/audit does with its lenses.

Read the target across the selected lenses, or all five when none was named. Read
`religion/context/findings.md` as well: quality findings recorded by /audit against
this area are candidates already identified, and folding them in avoids rediscovering them
under a second name. **Never write to the ledger.** /audit owns that file, and a
...
Write the spec into `religion/context/current-work.md`: the target, the lens, the green
...
Completion archives the campaign under `religion/history/refactors/`. Refactor campaigns
...
Then stop. This skill does not build. /implement does.
...
- **Never fix a bug in a refactor step.** A refactor that uncovers a genuine defect stops and
  points at /fix. Discovering that four copies of a function behave differently means
