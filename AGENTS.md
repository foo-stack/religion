...
Agent instructions for this repository, which is Religion's own source. Religion is
installed here and used to build itself: the plans, the active work item and the history
under `religion/` are real, not examples.

## What this is

A file-backed, spec-driven AI development workflow. Skills are authored once under
`src/skills/` and rendered into one tree per adapter. Never edit a rendered tree; edit the
source and rebuild.

<!-- religion:managed:start -->
...
<!-- religion:managed:end -->

## Commands

- Dev server: none, this is a library and a command-line tool
- Build: `npm run build`
- Test: `npm test`
- Verify: `npm test`
