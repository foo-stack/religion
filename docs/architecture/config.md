...
| `git.mode` | `trunk`, `branch-per-item` | `trunk` |
...
The three prefix settings only take effect under `git.mode: branch-per-item`. Under
`trunk` they are inert, and kept so that switching modes does not require reconfiguring.
