...
Use checkboxes. The build loop reads them: `feature` with no argument specs the
first unchecked item, and `complete` checks it off. That is what makes progress
...
stack, or deployment, update `project-plan.md` too, then re-run `overview`.
