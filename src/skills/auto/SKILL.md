...
- `{{cmd}}` - one work item, stopping **before** completing it, with a review packet.
- `{{cmd}} all` - the remaining build plan, completing each item, until it runs out or hits
  a bound.
- `{{cmd}} 3` - the next three items, completing each.
- `{{cmd}} resume` - continue an interrupted run from the file-backed state.
...
- Configuration is valid. Stop and point at {{cmd:doctor}} if not.
...
1. **Spec it**, following {{cmd:feature}}, including the sizing and the self-critique. Do
...
2. **Implement it**, following {{cmd:implement}}, with no prompt between steps. Verify each
...
5. **Complete it**, following {{cmd:complete}}, which archives, ticks the plan, prunes the
...
`{{cmd}} resume` picks up from there, because the state is in files rather than in the run.
