...
description: "Add a capability to {{product}} itself: interview for what the skill must do, write its source with the required frontmatter, add routing tests that prove the right skill is selected for a request, render it into every adapter tree, and verify. Refuses to add a skill whose job an existing one already does, since an overlapping roster routes badly and the cost lands on every later request. Use when the user runs {{cmd}}, asks to add a skill or command to the workflow, or asks to extend {{product}}."
---

# extend - add a capability to the workflow

This edits the workflow itself, not a project built with it. Run it in a {{product}} source
