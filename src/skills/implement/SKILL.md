...
description: "Build the feature, fix, or rollback spec'd in {{state}}/context/current-work.md, one small reviewable step at a time. Implements each step as the smallest change that satisfies its stated outcome, shows the diff and explains it in plain language, proves the outcome with evidence, ticks the step off so progress survives a cleared context, and resumes from the first unchecked step when interrupted. Repairs open high-severity findings as extra reviewed steps and has them re-reviewed independently. Ends with a review packet; the work commit and any merge belong to {{cmd:complete}}. Use when the user runs {{cmd}}, or asks to build, start, continue, or resume the current work item once its spec is ready."
---
...
**Read `{{state}}/config.json`.** A missing file means defaults. If it exists but cannot
be parsed, stop and point at {{cmd:doctor}}; a skill that is about to change code must not
guess past broken configuration.

**Read the spec.** If `{{state}}/context/current-work.md` is still the stub, or its status
is already complete, stop and say to run {{cmd:feature}}, {{cmd:fix}}, or
{{cmd:rollback}} first.

**Take the context.** Conventions from `{{state}}/context/coding-standards.md`, the data
...
4. **Verify what the step touched.** Run the focused checks for this change: the relevant
   ...
   stops and points at {{cmd:tests}}. With `when-configured`, the gate applies only when a
...
5. **Iterate until it works.** If it fails, revise and re-test. After two or three
   ...
   is failing, and offer to hand it to {{cmd:debug}}, which reproduces the failure and
...
Read `{{state}}/context/findings.md`. A P0 or P1 finding that is `open` or `fixed` will
...
Then have the repairs re-reviewed. Run {{cmd:audit}} over the repaired code **in a
...
- next action, normally {{cmd:complete}}
...
Then say that {{cmd:complete}} runs the full `Verify`, archives the spec, updates the
