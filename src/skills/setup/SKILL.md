...
it does not, leave that out; {{cmd:ci}} defines one deliberately later.
...
{{cmd:discovery}} for a guided session. Do not write plans from a scaffold; there is
...
- optional things worth knowing about, without pushing: {{cmd:tests}} if there is logic and
  no runner, {{cmd:ci}} if there are checks and no automation

Then stop. The next step is {{cmd:overview}}, once the plans are real.
