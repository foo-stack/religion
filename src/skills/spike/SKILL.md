...
description: "Answer a single question about whether an approach will work, by building the smallest throwaway thing that settles it inside a stated time budget, then deleting the code and recording the answer under {{state}}/history/spikes/. Nothing it writes ever ships. Use when the user runs {{cmd}}, asks whether something is possible or feasible, wants to know which of two approaches to take, says to try something and see, or is about to spec work that rests on an assumption nobody has tested. Not for a failure that already exists, which is {{cmd:debug}}, and not for how something should look, which is {{cmd:prototype}}."
...
## Where it sits
    ...
    a question the plans do not answer
      ...
      [spike]  -> answer recorded, code deleted
        |
    {{cmd:feature}} or {{cmd:fix}}, now speccing something known to work

## Input

One question, and ideally a budget:

    {{cmd}} "can we stream these updates over the platform's own APIs, or is a worker needed"
    {{cmd}} "is the vendor SDK usable from an edge runtime" --budget 1h
...
Record the result under `{{state}}/history/spikes/`, named for the question: what was asked,
