...
The spec for the work in progress lives in `{{state}}/context/current-work.md`.
...
6. **Prove it.** Run {{cmd:check}} for an outside proof pass against the running app.
7. **Review the code.** Run {{cmd:audit}} for a read-only quality pass. Findings go to the
   ledger with durable identifiers; repairs happen through {{cmd:implement}}.
8. **Complete.** Run {{cmd:complete}}. It runs the final safety pass, archives the spec
   under `{{state}}/history/`, checks the item off the build plan, resets the active spec,
   and makes the work commit. It ends with a manual walkthrough of what landed.
9. **Deploy, when it is time.** Run {{cmd:release}} after a completed item or a milestone
...
entry file, so {{cmd:implement}} continues from the first unchecked step. There is no
...
{{cmd:auto}} exists only as an explicit opt-in. Do not suggest it as the default next
action. When invoked it runs without pausing after each passing step, within the bounds in
`{{state}}/config.json`. Its invocation grants the routine actions listed in the Authority
